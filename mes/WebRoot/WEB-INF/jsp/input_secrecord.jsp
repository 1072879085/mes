...
<head>
	...
	//excel导出
	 ...
	 $.ajax({
					url : "${pageContext.request.contextPath}/record/importSecMaterialExcel.action",
					type : "POST",
					...
					if(data==1){
					layer.alert("辅料出库单excel导出成功!请在桌面查收文件！", {
		               ...
		               layer.closeAll();
		            	...
		            	layer.alert("辅料出库单excel导出失败!是否桌面已经存在该文件名！", {
		                skin: 'layui-layer-molv', //样式类名
		                closeBtn: 0
		            }, function(){
		               layer.closeAll();
		...
		<form id="forms" class="form-inline"  method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/record/toInputSecMaterialsRecord.action">
		  	...
		  	<div class="form-group">
		  		<label for="">车间名称</label>
		   		<input type="text" class="form-control"style="width:120px;" name="shopName"  value="">
		  	</div>
		  	<div class="form-group">
		  		<label for="">起始日期</label>
		    	<div class="input-group date form_date"  data-date-format="yyyy-mm-dd">
            		<input class="form-control" size="10" type="text" name="start_date" value=""/>
		  	...
		  	<div class="form-group">
		    	...
		    	<div class="input-group date form_date"  data-date-format="yyyy-mm-dd">
            		<input class="form-control" size="10" type="text" name="end_date" value=""/>
            		 <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
            		<span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
       			</div>
		  	</div>
		  ...
		  <button type="submit" class="btn btn-primary" >查询</button>
		    <!-- 选择导出的excelT+的模板 -->
			      <input type="file" id="excelPath" name="excelPath" class="form-control" style="width:150px;"/>
			      <input type="button" id="newmj" value="导出Excel" class="btn btn-success" onclick="excel()" />
		</form> 
		<br />
		<div class="container">	   
		<div class="table-responsive">
			...
			<!-- 循环输出forEach -->
					...
					<td>
					 ...
					 <input type="hidden" name="id" value="${a.getMaterialsId}"/>
					</td>	
					</tr>
					</c:forEach>	
				</tbody>
