    ...
    <h4 class="myClass navbar-left">设备管理</h4>
		...
		<form id="forms" class="form-inline "  method="post" enctype="multipart/form-data"  action="${pageContext.request.contextPath}/manage/togetAsset.action">
		  ...
		  <div class="form-group">
		    ...
		    <input type="text"  name="asset_name" class="form-control" id="equipmentName">
		  </div>
		  <div class="form-group">
		  	<label for="equipmentNum">设备编号</label>
		    <input type="text" name="asset_no" class="form-control" id="equipmentNum">
		  </div>
		  &nbsp; &nbsp;
	  ...
	  <table class="table">
			...
			<!-- 循环输出forEach -->
				...
				<%-- 	<td>${asset.director }</td> --%>
					...
					<td>
					<input type="hidden" name="id" value="${asset.assetId}"/><input class="btn  btn-xs btn-primary" type="button" value="查看详情" onclick="revise(this)"> 
					</td>
					<%-- <td>
					<input type="hidden" name="id" value="${asset.assetId}"/><input class="btn  btn-xs btn-primary" type="button" value="删除" onclick="revice(this)">
					</td> --%>
				</tr>
				</c:forEach>
			</tbody>
