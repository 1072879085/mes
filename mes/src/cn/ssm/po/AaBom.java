package cn.ssm.po;

import java.math.BigDecimal;
import java.util.Date;

public class AaBom {
    private String code;

    private String name;

    private String version;

    private BigDecimal producequantity;

    private BigDecimal rationwage;

    private BigDecimal rationcharge;

    private BigDecimal rationmanhour;

    private Byte isdefaultbom;

    private Integer bomdepth;

    private Byte disabled;

    private String updatedby;

    private String freeitem0;

    private String freeitem1;

    private String freeitem2;

    private String freeitem3;

    private String freeitem4;

    private String freeitem5;

    private String freeitem6;

    private String freeitem7;

    private String freeitem8;

    private String freeitem9;

    private String priuserdefnvc1;

    private BigDecimal priuserdefdecm1;

    private String priuserdefnvc2;

    private BigDecimal priuserdefdecm2;

    private String priuserdefnvc3;

    private BigDecimal priuserdefdecm3;

    private String priuserdefnvc4;

    private BigDecimal priuserdefdecm4;

    private String priuserdefnvc5;

    private BigDecimal priuserdefdecm5;

    private BigDecimal producequantity2;

    private BigDecimal rationcost;

    private BigDecimal rationmaterial;

    private BigDecimal cost;

    private BigDecimal rateofexchange;

    private BigDecimal yieldrate;

    private Byte iscostbom;

    private String idsaleorder;

    private String maker;

    private String auditor;

    private String reviser;

    private Byte isorderbom;

    private String customername;

    private String customercode;

    private String customerforshort;

    private String pubuserdefnvc1;

    private String pubuserdefnvc2;

    private String pubuserdefnvc3;

    private String pubuserdefnvc4;

    private String pubuserdefnvc5;

    private String pubuserdefnvc6;

    private BigDecimal pubuserdefdecm1;

    private BigDecimal pubuserdefdecm2;

    private BigDecimal pubuserdefdecm3;

    private BigDecimal pubuserdefdecm4;

    private BigDecimal pubuserdefdecm5;

    private BigDecimal pubuserdefdecm6;

    private BigDecimal priuserdefdecm6;

    private String priuserdefnvc6;

    private Integer id;

    private Integer idbomrelationdto;

    private Integer idmanufactureplant;

    private Integer idinventory;

    private Integer idmechiner;

    private Integer idrouting;

    private Integer idunit;

    private Integer idunit2;

    private Integer idwarehouse;

    private Integer getbomchildunitmethod;

    private Integer producetype;

    private Integer voucherstate;

    private Integer auditorid;

    private Integer makerid;

    private Date madedate;

    private Date updated;

    private Date voucherdate;

    private Date auditeddate;

    private Date createdtime;

    private Integer reviserid;

    private Date reviserdate;

    private byte[] ts;

    public String getCode() {
        return code;
    }

    public void setCode(String code) {
        this.code = code == null ? null : code.trim();
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name == null ? null : name.trim();
    }

    public String getVersion() {
        return version;
    }

    public void setVersion(String version) {
        this.version = version == null ? null : version.trim();
    }

    public BigDecimal getProducequantity() {
        return producequantity;
    }

    public void setProducequantity(BigDecimal producequantity) {
        this.producequantity = producequantity;
    }

    public BigDecimal getRationwage() {
        return rationwage;
    }

    public void setRationwage(BigDecimal rationwage) {
        this.rationwage = rationwage;
    }

    public BigDecimal getRationcharge() {
        return rationcharge;
    }

    public void setRationcharge(BigDecimal rationcharge) {
        this.rationcharge = rationcharge;
    }

    public BigDecimal getRationmanhour() {
        return rationmanhour;
    }

    public void setRationmanhour(BigDecimal rationmanhour) {
        this.rationmanhour = rationmanhour;
    }

    public Byte getIsdefaultbom() {
        return isdefaultbom;
    }

    public void setIsdefaultbom(Byte isdefaultbom) {
        this.isdefaultbom = isdefaultbom;
    }

    public Integer getBomdepth() {
        return bomdepth;
    }

    public void setBomdepth(Integer bomdepth) {
        this.bomdepth = bomdepth;
    }

    public Byte getDisabled() {
        return disabled;
    }

    public void setDisabled(Byte disabled) {
        this.disabled = disabled;
    }

    public String getUpdatedby() {
        return updatedby;
    }

    public void setUpdatedby(String updatedby) {
        this.updatedby = updatedby == null ? null : updatedby.trim();
    }

    public String getFreeitem0() {
        return freeitem0;
    }

    public void setFreeitem0(String freeitem0) {
        this.freeitem0 = freeitem0 == null ? null : freeitem0.trim();
    }

    public String getFreeitem1() {
        return freeitem1;
    }

    public void setFreeitem1(String freeitem1) {
        this.freeitem1 = freeitem1 == null ? null : freeitem1.trim();
    }

    public String getFreeitem2() {
        return freeitem2;
    }

    public void setFreeitem2(String freeitem2) {
        this.freeitem2 = freeitem2 == null ? null : freeitem2.trim();
    }

    public String getFreeitem3() {
        return freeitem3;
    }

    public void setFreeitem3(String freeitem3) {
        this.freeitem3 = freeitem3 == null ? null : freeitem3.trim();
    }

    public String getFreeitem4() {
        return freeitem4;
    }

    public void setFreeitem4(String freeitem4) {
        this.freeitem4 = freeitem4 == null ? null : freeitem4.trim();
    }

    public String getFreeitem5() {
        return freeitem5;
    }

    public void setFreeitem5(String freeitem5) {
        this.freeitem5 = freeitem5 == null ? null : freeitem5.trim();
    }

    public String getFreeitem6() {
        return freeitem6;
    }

    public void setFreeitem6(String freeitem6) {
        this.freeitem6 = freeitem6 == null ? null : freeitem6.trim();
    }

    public String getFreeitem7() {
        return freeitem7;
    }

    public void setFreeitem7(String freeitem7) {
        this.freeitem7 = freeitem7 == null ? null : freeitem7.trim();
    }

    public String getFreeitem8() {
        return freeitem8;
    }

    public void setFreeitem8(String freeitem8) {
        this.freeitem8 = freeitem8 == null ? null : freeitem8.trim();
    }

    public String getFreeitem9() {
        return freeitem9;
    }

    public void setFreeitem9(String freeitem9) {
        this.freeitem9 = freeitem9 == null ? null : freeitem9.trim();
    }

    public String getPriuserdefnvc1() {
        return priuserdefnvc1;
    }

    public void setPriuserdefnvc1(String priuserdefnvc1) {
        this.priuserdefnvc1 = priuserdefnvc1 == null ? null : priuserdefnvc1.trim();
    }

    public BigDecimal getPriuserdefdecm1() {
        return priuserdefdecm1;
    }

    public void setPriuserdefdecm1(BigDecimal priuserdefdecm1) {
        this.priuserdefdecm1 = priuserdefdecm1;
    }

    public String getPriuserdefnvc2() {
        return priuserdefnvc2;
    }

    public void setPriuserdefnvc2(String priuserdefnvc2) {
        this.priuserdefnvc2 = priuserdefnvc2 == null ? null : priuserdefnvc2.trim();
    }

    public BigDecimal getPriuserdefdecm2() {
        return priuserdefdecm2;
    }

    public void setPriuserdefdecm2(BigDecimal priuserdefdecm2) {
        this.priuserdefdecm2 = priuserdefdecm2;
    }

    public String getPriuserdefnvc3() {
        return priuserdefnvc3;
    }

    public void setPriuserdefnvc3(String priuserdefnvc3) {
        this.priuserdefnvc3 = priuserdefnvc3 == null ? null : priuserdefnvc3.trim();
    }

    public BigDecimal getPriuserdefdecm3() {
        return priuserdefdecm3;
    }

    public void setPriuserdefdecm3(BigDecimal priuserdefdecm3) {
        this.priuserdefdecm3 = priuserdefdecm3;
    }

    public String getPriuserdefnvc4() {
        return priuserdefnvc4;
    }

    public void setPriuserdefnvc4(String priuserdefnvc4) {
        this.priuserdefnvc4 = priuserdefnvc4 == null ? null : priuserdefnvc4.trim();
    }

    public BigDecimal getPriuserdefdecm4() {
        return priuserdefdecm4;
    }

    public void setPriuserdefdecm4(BigDecimal priuserdefdecm4) {
        this.priuserdefdecm4 = priuserdefdecm4;
    }

    public String getPriuserdefnvc5() {
        return priuserdefnvc5;
    }

    public void setPriuserdefnvc5(String priuserdefnvc5) {
        this.priuserdefnvc5 = priuserdefnvc5 == null ? null : priuserdefnvc5.trim();
    }

    public BigDecimal getPriuserdefdecm5() {
        return priuserdefdecm5;
    }

    public void setPriuserdefdecm5(BigDecimal priuserdefdecm5) {
        this.priuserdefdecm5 = priuserdefdecm5;
    }

    public BigDecimal getProducequantity2() {
        return producequantity2;
    }

    public void setProducequantity2(BigDecimal producequantity2) {
        this.producequantity2 = producequantity2;
    }

    public BigDecimal getRationcost() {
        return rationcost;
    }

    public void setRationcost(BigDecimal rationcost) {
        this.rationcost = rationcost;
    }

    public BigDecimal getRationmaterial() {
        return rationmaterial;
    }

    public void setRationmaterial(BigDecimal rationmaterial) {
        this.rationmaterial = rationmaterial;
    }

    public BigDecimal getCost() {
        return cost;
    }

    public void setCost(BigDecimal cost) {
        this.cost = cost;
    }

    public BigDecimal getRateofexchange() {
        return rateofexchange;
    }

    public void setRateofexchange(BigDecimal rateofexchange) {
        this.rateofexchange = rateofexchange;
    }

    public BigDecimal getYieldrate() {
        return yieldrate;
    }

    public void setYieldrate(BigDecimal yieldrate) {
        this.yieldrate = yieldrate;
    }

    public Byte getIscostbom() {
        return iscostbom;
    }

    public void setIscostbom(Byte iscostbom) {
        this.iscostbom = iscostbom;
    }

    public String getIdsaleorder() {
        return idsaleorder;
    }

    public void setIdsaleorder(String idsaleorder) {
        this.idsaleorder = idsaleorder == null ? null : idsaleorder.trim();
    }

    public String getMaker() {
        return maker;
    }

    public void setMaker(String maker) {
        this.maker = maker == null ? null : maker.trim();
    }

    public String getAuditor() {
        return auditor;
    }

    public void setAuditor(String auditor) {
        this.auditor = auditor == null ? null : auditor.trim();
    }

    public String getReviser() {
        return reviser;
    }

    public void setReviser(String reviser) {
        this.reviser = reviser == null ? null : reviser.trim();
    }

    public Byte getIsorderbom() {
        return isorderbom;
    }

    public void setIsorderbom(Byte isorderbom) {
        this.isorderbom = isorderbom;
    }

    public String getCustomername() {
        return customername;
    }

    public void setCustomername(String customername) {
        this.customername = customername == null ? null : customername.trim();
    }

    public String getCustomercode() {
        return customercode;
    }

    public void setCustomercode(String customercode) {
        this.customercode = customercode == null ? null : customercode.trim();
    }

    public String getCustomerforshort() {
        return customerforshort;
    }

    public void setCustomerforshort(String customerforshort) {
        this.customerforshort = customerforshort == null ? null : customerforshort.trim();
    }

    public String getPubuserdefnvc1() {
        return pubuserdefnvc1;
    }

    public void setPubuserdefnvc1(String pubuserdefnvc1) {
        this.pubuserdefnvc1 = pubuserdefnvc1 == null ? null : pubuserdefnvc1.trim();
    }

    public String getPubuserdefnvc2() {
        return pubuserdefnvc2;
    }

    public void setPubuserdefnvc2(String pubuserdefnvc2) {
        this.pubuserdefnvc2 = pubuserdefnvc2 == null ? null : pubuserdefnvc2.trim();
    }

    public String getPubuserdefnvc3() {
        return pubuserdefnvc3;
    }

    public void setPubuserdefnvc3(String pubuserdefnvc3) {
        this.pubuserdefnvc3 = pubuserdefnvc3 == null ? null : pubuserdefnvc3.trim();
    }

    public String getPubuserdefnvc4() {
        return pubuserdefnvc4;
    }

    public void setPubuserdefnvc4(String pubuserdefnvc4) {
        this.pubuserdefnvc4 = pubuserdefnvc4 == null ? null : pubuserdefnvc4.trim();
    }

    public String getPubuserdefnvc5() {
        return pubuserdefnvc5;
    }

    public void setPubuserdefnvc5(String pubuserdefnvc5) {
        this.pubuserdefnvc5 = pubuserdefnvc5 == null ? null : pubuserdefnvc5.trim();
    }

    public String getPubuserdefnvc6() {
        return pubuserdefnvc6;
    }

    public void setPubuserdefnvc6(String pubuserdefnvc6) {
        this.pubuserdefnvc6 = pubuserdefnvc6 == null ? null : pubuserdefnvc6.trim();
    }

    public BigDecimal getPubuserdefdecm1() {
        return pubuserdefdecm1;
    }

    public void setPubuserdefdecm1(BigDecimal pubuserdefdecm1) {
        this.pubuserdefdecm1 = pubuserdefdecm1;
    }

    public BigDecimal getPubuserdefdecm2() {
        return pubuserdefdecm2;
    }

    public void setPubuserdefdecm2(BigDecimal pubuserdefdecm2) {
        this.pubuserdefdecm2 = pubuserdefdecm2;
    }

    public BigDecimal getPubuserdefdecm3() {
        return pubuserdefdecm3;
    }

    public void setPubuserdefdecm3(BigDecimal pubuserdefdecm3) {
        this.pubuserdefdecm3 = pubuserdefdecm3;
    }

    public BigDecimal getPubuserdefdecm4() {
        return pubuserdefdecm4;
    }

    public void setPubuserdefdecm4(BigDecimal pubuserdefdecm4) {
        this.pubuserdefdecm4 = pubuserdefdecm4;
    }

    public BigDecimal getPubuserdefdecm5() {
        return pubuserdefdecm5;
    }

    public void setPubuserdefdecm5(BigDecimal pubuserdefdecm5) {
        this.pubuserdefdecm5 = pubuserdefdecm5;
    }

    public BigDecimal getPubuserdefdecm6() {
        return pubuserdefdecm6;
    }

    public void setPubuserdefdecm6(BigDecimal pubuserdefdecm6) {
        this.pubuserdefdecm6 = pubuserdefdecm6;
    }

    public BigDecimal getPriuserdefdecm6() {
        return priuserdefdecm6;
    }

    public void setPriuserdefdecm6(BigDecimal priuserdefdecm6) {
        this.priuserdefdecm6 = priuserdefdecm6;
    }

    public String getPriuserdefnvc6() {
        return priuserdefnvc6;
    }

    public void setPriuserdefnvc6(String priuserdefnvc6) {
        this.priuserdefnvc6 = priuserdefnvc6 == null ? null : priuserdefnvc6.trim();
    }

    public Integer getId() {
        return id;
    }

    public void setId(Integer id) {
        this.id = id;
    }

    public Integer getIdbomrelationdto() {
        return idbomrelationdto;
    }

    public void setIdbomrelationdto(Integer idbomrelationdto) {
        this.idbomrelationdto = idbomrelationdto;
    }

    public Integer getIdmanufactureplant() {
        return idmanufactureplant;
    }

    public void setIdmanufactureplant(Integer idmanufactureplant) {
        this.idmanufactureplant = idmanufactureplant;
    }

    public Integer getIdinventory() {
        return idinventory;
    }

    public void setIdinventory(Integer idinventory) {
        this.idinventory = idinventory;
    }

    public Integer getIdmechiner() {
        return idmechiner;
    }

    public void setIdmechiner(Integer idmechiner) {
        this.idmechiner = idmechiner;
    }

    public Integer getIdrouting() {
        return idrouting;
    }

    public void setIdrouting(Integer idrouting) {
        this.idrouting = idrouting;
    }

    public Integer getIdunit() {
        return idunit;
    }

    public void setIdunit(Integer idunit) {
        this.idunit = idunit;
    }

    public Integer getIdunit2() {
        return idunit2;
    }

    public void setIdunit2(Integer idunit2) {
        this.idunit2 = idunit2;
    }

    public Integer getIdwarehouse() {
        return idwarehouse;
    }

    public void setIdwarehouse(Integer idwarehouse) {
        this.idwarehouse = idwarehouse;
    }

    public Integer getGetbomchildunitmethod() {
        return getbomchildunitmethod;
    }

    public void setGetbomchildunitmethod(Integer getbomchildunitmethod) {
        this.getbomchildunitmethod = getbomchildunitmethod;
    }

    public Integer getProducetype() {
        return producetype;
    }

    public void setProducetype(Integer producetype) {
        this.producetype = producetype;
    }

    public Integer getVoucherstate() {
        return voucherstate;
    }

    public void setVoucherstate(Integer voucherstate) {
        this.voucherstate = voucherstate;
    }

    public Integer getAuditorid() {
        return auditorid;
    }

    public void setAuditorid(Integer auditorid) {
        this.auditorid = auditorid;
    }

    public Integer getMakerid() {
        return makerid;
    }

    public void setMakerid(Integer makerid) {
        this.makerid = makerid;
    }

    public Date getMadedate() {
        return madedate;
    }

    public void setMadedate(Date madedate) {
        this.madedate = madedate;
    }

    public Date getUpdated() {
        return updated;
    }

    public void setUpdated(Date updated) {
        this.updated = updated;
    }

    public Date getVoucherdate() {
        return voucherdate;
    }

    public void setVoucherdate(Date voucherdate) {
        this.voucherdate = voucherdate;
    }

    public Date getAuditeddate() {
        return auditeddate;
    }

    public void setAuditeddate(Date auditeddate) {
        this.auditeddate = auditeddate;
    }

    public Date getCreatedtime() {
        return createdtime;
    }

    public void setCreatedtime(Date createdtime) {
        this.createdtime = createdtime;
    }

    public Integer getReviserid() {
        return reviserid;
    }

    public void setReviserid(Integer reviserid) {
        this.reviserid = reviserid;
    }

    public Date getReviserdate() {
        return reviserdate;
    }

    public void setReviserdate(Date reviserdate) {
        this.reviserdate = reviserdate;
    }

    public byte[] getTs() {
        return ts;
    }

    public void setTs(byte[] ts) {
        this.ts = ts;
    }
}