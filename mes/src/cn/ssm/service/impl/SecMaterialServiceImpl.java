...
public class SecMaterialServiceImpl implements SecMaterialService {
	...
	//8.查看
	   ...
	   //9.领辅料查询：根据车间、起始日期、截止日期、（隐藏条件是否领料？is_return=0）查询
		...
		@Override
		public List<InputSec> selectexcelinputSecGetMaterial() {
			
			return getSecMaterialsMapper.selectexcelinputSecGetMaterial();
		}
		
		//12.退取辅料（excel导出当日的信息为T+模板）
		@Override
		public List<GetSecDetail> selectexceloutputSecGetMaterial() {
			
			return getSecDetailMapper.selectexceloutputSecGetMaterial();
		}
