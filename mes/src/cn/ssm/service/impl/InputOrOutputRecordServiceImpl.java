...
public class InputOrOutputRecordServiceImpl implements IntputOrOutputRecordService{
	...
	//11.1原材料T+系统的选择模板并选择模板excel导出（除了外协的原材料出库）
	@Override
	public List<Input> selectRowinMaterial(){
		List<Input> listrowinMaterial=new ArrayList<Input>();
		listrowinMaterial=getMaterialMapper.selectRowinMaterial();
		return listrowinMaterial;
	}

	//11.2退原材料T+系统的选择模板并选择模板excel导出（除了外协的原材料出库）
	@Override
	public List<Output> selectRowoutMaterial() {
		List<Output> listrowoutMaterial=new ArrayList<Output>();
		listrowoutMaterial=getMaterialMapper.selectRowoutMaterial();
		return listrowoutMaterial;
	}

	// 12.成品记录T+系统的选择模板并选择模板excel导出
	@Override
	public List<ProductRecord> selectexcelOutputFullRecord() {
	
		return productRecordMapper.selectexcelOutputFullRecord();
	}
